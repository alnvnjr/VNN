...
int main(){
    ...
    printf("Using one out of %d devices\n", ret_num_devices);
    ret = clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_DEFAULT, 1, &device_id, &ret_num_devices);
...
#error "unknown openCL SDK environment"
    ...
    // Create Kernel Program from binary
...
#error "unknown OpenCL SDK environment"
    ...
    // Buffer management on the host side
    ...
    /* We assume A, B, and C are float arrays which
    have been declared and initiatized.
    */
   ...
   // Declare Buffers and Move data
    ...
    //Execute the kernel
    //Set the work group and work size
    ...
    // It is important to check the return value, for example, when enqueueNDRangeKernel may fail when work roup size does not divide evenly into global work size
    if (ret != CL_SUCCESS) {
